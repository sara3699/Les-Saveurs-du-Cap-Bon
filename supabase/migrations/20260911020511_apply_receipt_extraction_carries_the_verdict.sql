-- The reading, and the verdict on it, written in one go.
--
-- The verdict is decided in src/lib/receipts/validate.ts and arrives here already made.
-- Both outcomes write everything that was read: a refused receipt keeps its supplier and
-- its date even though its total is missing, so the person filling the gap fills one box
-- rather than the whole form. What refusing changes is the status and the reasons, not
-- the contents.
--
-- Replaces the version in 20260910235826. Same eighteen columns, plus the two the
-- verdict needs.

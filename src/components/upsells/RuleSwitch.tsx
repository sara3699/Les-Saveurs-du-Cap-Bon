    ...
    <div className="mt-auto flex flex-col gap-1.5 border-t border-line pt-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          role="switch"
          aria-checked={on}
          aria-label={`${on ? "Désactiver" : "Activer"} ${ruleName}`}
          onClick={() => setOn(!on)}
      ...
      <p role="status" className="text-[11.5px] text-muted">
        {changed
          ? "Modifié sur cet écran uniquement, rien n'est enregistré."
          : "Déplacer cet interrupteur change cet écran uniquement, rien n'est enregistré."}
      </p>
    </div>
  );
}

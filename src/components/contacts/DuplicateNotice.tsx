  ...
  return (
    <section className="rounded-[var(--radius-card)] border border-accent-line bg-accent-soft p-4 text-accent-ink sm:p-5">
      <h2 className="text-[15px] leading-tight text-accent-ink">
        {pairs.length === 1 ? (
          "Une paire de fiches pourrait etre la même personne"
        ) : (
          <>
            <span className="os-num">{pairs.length}</span> paires de fiches pourraient chacune etre
            la même personne
          </>
        )}
      </h2>
      <p className="mt-1 text-[13px]">
        Un import de tableur a laisse une deuxieme fiche derriere lui.{" "}
        {allShown
          ? "Chaque fiche nommee ici est dans la liste ci-dessous, et chacune y est signalee."
      ...
      <p className="mt-3 border-t border-accent-line pt-3 text-[12px]">
        ...
        encore de bouton de fusion ici. En attendant, continuez a travailler depuis la fiche qui
        porte les commandes.
      </p>
    </section>
  );

-- A QR code is scanned at the counter, before anything is read off the paper. Keeping
-- it at that moment rather than at extraction means a failed read does not lose it, and
-- nobody has to hold the phone over the receipt a second time.
--
-- What arrives here has already been through parseQrPayload on the server: the raw
-- string is capped and stripped of control characters, and the address, if there is
-- one, is https to a public host or it is not here at all.
...
declare
  ...
  v_qr_kind text := left(nullif(trim(p_payload ->> 'qr_kind'), ''), 40);
  v_qr_url text := left(nullif(trim(p_payload ->> 'verification_url'), ''), 2000);
  v_existing uuid;
  v_recent integer;
  v_receipt uuid;
begin
